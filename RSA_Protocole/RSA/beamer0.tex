...
\transwipe
Deux protocoles pour deux utilisations différentes développés par M. Bellare et P. Rogaway :
\begin{description}
\pause
...
\transwipe
\begin{proposition}
	Le protocole OAEP n'est pas totalement sémantiquement sûr. 
\end{proposition}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
...
\begin{theoreme}
S'il existe un schéma xor-malléable alors il existe une permutation à sens-unique avec trappe, telle que lorsqu'on utilise OEAP le schéma de chiffrement qui en résulte n'est pas sûr dans un modèle d'oracle aléatoire.
\end{theoreme}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[shrink]
...
L'attaquant reçoit le chalenge $y'$ , il peut l'écrire tel que $y' = s'\parallel f_{0}(t')$.\\
...
\end{array}
\\
Si l'attaquant déchiffre $y$ à l'aide d'un oracle aléatoire, alors il obtient $x$.\\
...
\transwipe
\begin{theoreme}
Il existe un oracle aléatoire tel qu'une permutation à sens-unique avec trappe existe.
\end{theoreme}

La deux théorèmes précédents nous donnent le corollaire suivant :

\begin{corollaire}
Il existe un oracle aléatoire tel que la construction d'OAEP n'est pas sémantiquement sûr.
\end{corollaire}
\end{frame}
...
\pause
\begin{exemple}
Pour une clé de 1024 bits, la complexité de la réduction est de $2^{40}$.
\end{exemple}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{RSASSA-PSS}
...
\transwipe
Les deux protocoles ont une sécurité sémantique partielle.
\begin{description}
\item[OAEP] Il est préférable de ne plus utiliser OAEP, et plutôt REACT ou d'autres protocoles de PKCS $\#1.5$,
